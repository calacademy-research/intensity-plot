...
// }
    ...
    render() {
        return (
            ...
            <div id="graph_wrapper" className="noselect"> 
                ...
                <div id="all_graphs">
                {this.items.map( (item, i) => GridGraph.renderGraph(i, item))}
                </div>
            
            </div>
                
                <canvas id="tip" width="170" height="20"/>
            </div>
        );
    }

    static renderGraph(i, graph) {
            let id = "graph_and_labels"+i;
            let graphContainerId = "graph_container"+i;
            let mainCanvasContainerId = "main_canvas_container"+i;
            let canvasId = "main_canvas"+i;
            let canvasClass = "canvas"+i;
            let canvasBackgroundId = "canvas_background"+i;
            let namesCanvas = "sample_name"+i;
            let labelCanvasId = "length_label"+i;
            let barCanvasId = "hit_bars"+i;

            return (
                <div id={{id}} style="vertical-align: top; padding-top: 6px; ">
                    <div id={{graphContainerId}}>
                        <div id={{mainCanvasContainerId}} style="display: inline-flex;">
                            <canvas id={{canvasId}} className={{canvasClass}} style="position:relative;"/>
                            <canvas id={{canvasBackgroundId}} style="position:relative; "/>
                        </div>
                        <canvas id={{namesCanvas}} className={{canvasClass}} style="position:relative;"/>
                    </div>
                    <canvas id={{labelCanvasId}} className={{canvasClass}}/>
                    <br/>
                    <canvas id={{barCanvasId}} className={{canvasClass}}/>
                </div>
            )
        }
}
