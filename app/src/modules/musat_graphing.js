...
function fin_callLoci() {
    ...
    $("#" + g.primer_nm + " .plotPrimer").removeClass("processing"); // make name blue after processing
    setCalled();
    $("#msg").empty();
}
...
function callSample(sample, alwaysCall) { // if alwaysCall is false, only set peaks if none set already
    ...
    mu_setOLocusCallsFromSampleGraph(g.primer_nm,sample, plot.series[1].data); // 20May2016 JBH set array of called pts for sample (adding objects in mu global as necessary)
...
function makeNthPlot(style, pop_x_scale) {
    ...
    function arwSpan(popnum) {
        ...
        var nxtclass = (popnum >= (g.num_pops)) ? "class='disable'" : "class='popnav' onclick='onNextPop(event)'";
...
function drawPlot(sample, n, pop_x_scale) { // passed in name of a sample
    ...
    if (pop_x_scale) { // force samples in same population to use the same x-scale
        ...
        len_max = g.primer_max + 1;
        len_min = g.primer_min - 1
    }
    ...
    if (!mu.oLocusCalls[g.primer_nm]) {
        mu.oLocusCalls[g.primer_nm] = {};
    }
    var calledPts = mu.oLocusCalls[g.primer_nm][sample] || [];
    ...
    function deletePt(ev, seriesIndex, pointIndex, data) {
        if (info.length > 1 && window.confirm("Delete [" + data + "]?")) {
    ...
    function toggleLocusSelection(ev, seriesIndex, pointIndex, data) {
        ...
        mu_setOLocusCallsFromSampleGraph(g.primer_nm,sample, plot.series[1].data);
...
function setCalled() { // set indicator for curfile's primer_nm as to whether called and how many samples called
    updateAlleleCountsDisplay('#explore_bars', g.primer_nm, true);
}
...
function gr_injectHtml() {
    ...
    var filepart = g.curfile.split('\\').pop().split('/').pop();
    ...
    var cap = "<div>" + g.primer_nm + " &nbsp;Microsatellite Distributions for " + (names.length).toString() + " samples from " + g.num_pops.toString() + " populations" +// in " + filepart +
        vwbtn + "</div><div><span id='msg'>Creating Plots...</span> &nbsp;<span class='cmds'>" + cmds + "</span></div>";
    $("#gr_caption").html(cap);
    $("#" + g.primer_nm).addClass("processing") // make name red and italic while processing
}
...
export {clearCalledIndicator, setCalledIndicators}
