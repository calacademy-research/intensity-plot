...
import {initTraitMinMax} from "./global"
import {g, hits, names, sample_pts_trimmed} from "./global"
import {nSQL} from "@nano-sql/core";

// simple database with single table (not normalized) to assist with querying
// and persisting
var createDatabase = nSQL().createDatabase({
    ...
    tables: [
        { // tables can be created as part of createDatabase or created later with create table queriesname: "samples", model: {
            "id:uuid": {
                pk: true
            },
            "name:string": {},
...
// Extracts an array of sample names (into global "Names") and Maintains a
// running minimum and maximum trait values to further refine data extents
    ...
    createDatabase.then(() => {
        ...
        for (var sampleName in hits) {
            ...
            for (var i = 0; i < samples.length; i++) {
                var item = samples[i];
                promises.push(nSQL("samples").query("upsert", {
                    name: sampleName,
                    trait: item[0],
                    intensity: item[1]
                }).exec())
...
function parseData(ary) {
    ...
    // sort the sample names from the Allele Calls file by their Population if we
    // have that set otherwise sort by Sample name itself and everything is in one
    // big GenPop
    names.sort(sortByPopThenSample);
    gr_refineXAxis(); // 10May2016 JBH trim low count reads from sample allele len extrema
}

function gr_refineXAxis() {

    // 10May2016 JBH new method to size x-axis for all the plots created for this primer file
    // sample hits assumed to have been collected in hits[sample] array We make 2
    // passes through each sample: 1st pass sets allele lens for min and for max
    // where the count for the len is > 2% of the len with the greatest number of
    // reads for this sample. As we go through this 1st pass we set the global
    // trait_min and trait_max if this sample's min or max pts extend them. 2nd pass
    // uses this global min and max to trim each sample's left points and right
    // points that don't fit

    initTraitMinMax();

    // 1st pass: set trait_min and trait_max based on each sample's local min and
    // max (currently 2% of sample's highest peak aka read count)
    for (let s = 0; s < names.length; s++) {
        ...
        if (local_extrema) { // adjust global settings if this sample had high enough min or max to make x-axis wider
            if (local_extrema.min < g.trait_min) 
                g.trait_min = local_extrema.min
            if (local_extrema.max > g.trait_max) 
                g.trait_max = local_extrema.max
        }
    }

    // 2nd pass: go through the hits and remove points in hits[sample] that are
    // outside global trait_min and trait_max allele lens
    for (let s = 0; s < names.length; s++) {
        let sample = names[s];
        ...
        trimLowConfHits(sample, g.trait_min, g.trait_max);
    }
}
...
function trimLowConfHits(sample, trait_min, trait_max) {
    // remove low_conf hits for this sample and store them in
    // sample_pts_trimmed[sample].left[]  and sample_pts_trimmed[sample].right[]
    ...
    for (var r = (pts.length) - 1; r >= 0; r--) { // remove pts at end of array whose lens are < trait_max
        if (pts[r][0] > right_edge) { // len outside of rightmost plot line, read count < what fits on plot, delete this point
            ...
            // stop at first point that needs to be plotted on graph
            break
    }
    while (pts.length > 0) { // remove pts at start of array whose lens are < trait_min
        if (pts[0][0] < left_edge) { // len outside of leftmost plot line, read count < what fits on plot, delete this point
            ...
            // stop at first point that needs to be ploteed
            break
    }
}
